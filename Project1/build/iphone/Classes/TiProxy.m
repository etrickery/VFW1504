...
//Rare exceptions to indicate a bug in the _quoteviewer code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_quoteviewer = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_quoteviewer]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.quoteviewer.module"
